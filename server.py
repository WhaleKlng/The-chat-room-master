...
class ChatServer(threading.Thread):
    ...
    def tcp_connect(self, conn, addr):
        ...
        if user == 'no': #测试用的 目的为检查ip（同局域网下聊天）
            user = addr[0] + ':' + str(addr[1])
        users.append((conn, user, addr))
        print(' New connection:', addr, ':', user)         # 打印用户名 不换行
        d = onlines()                                                   # 有新连接则刷新客户端的在线用户显示
        self.recv(d, addr)
        #主逻辑函数 一些辅助函数写在了后面  目的while监听之
        try:
            while True:
                data = conn.recv(1024)
                data = data.decode()
    ...
    def sendData(self):
        while True:
            if not que.empty():
                data = ''
                reply_text = ''
                message = que.get()                               # 取出队列第一个元素 注意上面的写法 (addr, data)决定了返回的是data
                if isinstance(message[1], str):                   # 如果data是str则返回Ture
                    for i in range(len(users)):
                        # users[i][1]是用户名, users[i][2]是addr, 将message[0]改为用户名   eg：users.append((conn, user, addr))
                        for j in range(len(users)):
                            if message[0] == users[j][2]: #add=add?
                                # print(' this message is from user[{}]'.format(j))  print都是测试用的可以删除掉的
                                print(' User[{}]'.format(j) + '||'+users[j][1]+':   '+ message[1].split(':;')[0])
                                # if '@Robot' in message[1] and reply_text == '':
                                #
                                #     msg = message[1].split(':;')[0]
                                #     reply = call_robot(url, apikey, msg) #调用的api
                ...
                if isinstance(message[1], list):  # 同上
                    ...
                    for i in range(len(users)):
                        try:
                            users[i][0].send(data.encode()) #conn的对象
                        except:
                            pass
...
class FileServer(threading.Thread):
    ...
    def run(self):
        self.s.bind(self.ADDR)
        print('File server listing at',self.s.getsockname())
        self.s.listen(3)
        while True:
            conn, addr = self.s.accept()
            t = threading.Thread(target=self.tcp_connect, args=(conn, addr))
...
class PictureServer(threading.Thread):
    ...
    def run(self):
        self.s.bind(self.ADDR)
        self.s.listen(5)
        print('Picture server listening at',self.s.getsockname())
        while True:
            conn, addr = self.s.accept()
            t = threading.Thread(target=self.tcp_connect, args=(conn, addr))
            t.start()
